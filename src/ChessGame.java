...
public class ChessGame {
  ...
  public boolean isInCheck(PieceColor roiColor) {
      Position roiPosition = findRoiPosition(roiColor);
      for (int row = 0; row < board.getBoard().length; row++) {
          for (int col = 0; col < board.getBoard()[row].length; col++) {
              Piece piece = board.getPiece(row, col);
              if (piece != null && piece.getColor() != roiColor) {
                  if (piece.isValidMove(roiPosition, board.getBoard())) {
                      return true;
                  }
              }
          }
  ...
  private Position findRoiPosition(PieceColor color) {
      for (int row = 0; row < board.getBoard().length; row++) {
          for (int col = 0; col < board.getBoard()[row].length; col++) {
              Piece piece = board.getPiece(row, col);
      ...
      throw new RuntimeException("Roi non trouvé, cela ne devrait pas arriver.");
  }

  public boolean isCheckmate(PieceColor roiColor) {
      if (!isInCheck(roiColor)) {
          return false;
      }

      Position roiPosition = findRoiPosition(roiColor);
      Roi roi = (Roi) board.getPiece(roiPosition.getRow(), roiPosition.getColumn());
      ...
      for (int rowOffset = -1; rowOffset <= 1; rowOffset++) {
          for (int colOffset = -1; colOffset <= 1; colOffset++) {
              ...
              Position newPosition = new Position(roiPosition.getRow() + rowOffset,
                      roiPosition.getColumn() + colOffset);

              if (isPositionOnBoard(newPosition) && roi.isValidMove(newPosition, board.getBoard())
                      && !wouldBeInCheckAfterMove(roiColor, roiPosition, newPosition)) {
                  return false;
              }
          }
      }
  ...
  private boolean wouldBeInCheckAfterMove(PieceColor roiColor, Position from, Position to) {
      ...
      boolean inCheck = isInCheck(roiColor);
  ...
  public List<Position> getLegalMovesForPieceAt(Position position) {
      ...
      switch (selectedPiece.getClass().getSimpleName()) {
          case "Pion":
              addPionMoves(position, selectedPiece.getColor(), legalMoves);
              break;
          case "Tour":
  ...
  private void addPionMoves(Position position, PieceColor color, List<Position> legalMoves) {
